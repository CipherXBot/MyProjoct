...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        sys.modules["cipherxbot.events"] = cipherxbot.utils
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["cipherxbot.plugins." + shortname] = mod
...
def remove_plugin(shortname):
    try:
        try:
            ...
            name = f"cipherxbot.plugins.{shortname}"
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            ftext += "--------BEGIN CIPHERXBOT TRACEBACK LOG--------"
            ftext += "\nDate: " + date
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"cipherxbot/plugins/assistant/{shortname}.py")
        name = "cipherxbot.plugins.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        spec.loader.exec_module(mod)
        sys.modules["cipherxbot.plugins.assistant" + shortname] = mod
